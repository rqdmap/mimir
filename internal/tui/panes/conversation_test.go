...
func TestConversationRenderNoPanic(t *testing.T) {
	...
	cp := panes.NewConversationPane(120, 40, panes.DefaultTheme)
...
func TestConversationEmptyState(t *testing.T) {
	cp := panes.NewConversationPane(80, 24, panes.DefaultTheme)
	view := cp.View()
...
func TestConversationToolOnlySession(t *testing.T) {
	...
	cp := panes.NewConversationPane(100, 30, panes.DefaultTheme)
	cp = applySetMessages(cp, messages)
	view := cp.View()
